...
class FuelElement:
    ...
    Provides feature specifications (cladding, fuel meat, Zr fill rod) and a
    nested ``Pincell`` class that constructs the concentric cylindrical model.
    ...
    class Pincell(CylindricalPinCell):
        ...
        @property
        def cladding(self) -> FuelElement.Cladding:
            return self._cladding

        @property
        def fuel_meat(self) -> FuelElement.FuelMeat:
            return self._fuel_meat

        @property
        def zr_fill_rod(self) -> FuelElement.ZrFillRod:
