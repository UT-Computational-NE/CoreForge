...
def test_zero_gap_profile(default_features):
    ...
    expected_materials = [
        isinstance(pin.zones[0].material, Zr),
        isinstance(pin.zones[1].material, UZrH),
        isinstance(pin.zones[2].material, SS304),
    ]
...
def test_gaps_included_when_above_tol():
    ...
    fuel = FuelElement.FuelMeat(inner_radius=0.15, outer_radius=0.50)
    clad = FuelElement.Cladding(thickness=0.10, outer_radius=0.80)
